...
menu(title='7-Zip' mode='multiple' type='file|dir|drive|back' image=cmd_7zipA) 
{
	...
	menu(title='Archive...' where=mkeyANY image=inherit expanded=1) 
	{
		...
		item(title='Extract...' keys='Smart II' mode='multiple'
			cmd-ps = `$files=@sel(2, ','); $smartFiles=@@(); $normalFiles=@@(); foreach ($file in $files) { $isSmart = ((& '@cmd_7zipC' l $file | Where-Object { $_ -match '^\d{4}-\d{2}-\d{2}' -and $_ -match 'D\.{4}' -and $_ -notmatch '\\' }).Count -eq 1); if ($isSmart) { $smartFiles += $file } else { $normalFiles += $file } }; $smartFiles | ForEach-Object { & '@cmd_7zipG' x $_ -spe }; $normalFiles | ForEach-Object { & '@cmd_7zipG' x $_ -o*\ -spe }`
						window=0)
